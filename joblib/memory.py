...
from .logger import Logger, format_time, pformat, _get_child_logger
    ...
    def _cached_call(self, args, kwargs, shelving=False):
        ...
        if not (self._check_previous_func_code(stacklevel=4) and
            ...
            try:
                ...
                _, name = get_func_name(self.func)
                self._logger.debug(
                    '%s cache loaded - %s', name, format_time(t)
                )
            except Exception:
...
class Memory(Logger):
    ...
    def __init__(self, location=None, backend='local', cachedir=None,
        ...
        # create a passthrough-logger associated to the cache directory
        # (used across MemorizedFunc, MemorizedResult...)
        _base_logger = logger.getChild(self.location or '')
        _base_logger.setLevel(logging.NOTSET)
